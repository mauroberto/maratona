...
BigInteger potenciaRapida(BigInteger b, BigInteger p){
	if(p == BigInteger("1")){
		return b;
	}else if(p == BigInteger("0")){
		return BigInteger("1");
	}else{
		BigInteger div = p/BigInteger("2");
		BigInteger parcial = potenciaRapida(b, div);
		if(div%BigInteger("2") == BigInteger("0")){
			return parcial*parcial;
		}else{
			return parcial*parcial*b;
		}
	}
}
...
int main() {
	int n;
	int bacteria = 0;
	BigInteger bi = BigInteger("0");
    	scanf("%d", &n);
	for(int i=0; i<n; i++){
		int b,p;
		scanf("%d%d", &p, &b);
		BigInteger r = potenciaRapida(BigInteger(b), BigInteger(p));
